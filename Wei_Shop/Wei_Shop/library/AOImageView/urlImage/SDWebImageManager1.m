...
#import "SDWebImageManager1.h"
#import "SDImageCache1.h"
#import "SDWebImageDownloader1.h"
#import "UrlImageView.h"
static SDWebImageManager1 *instance;

@implementation SDWebImageManager1
...
+ (id)sharedManager
{
    if (instance == nil)
    {
        instance = [[SDWebImageManager1 alloc] init];
    }
...
{
    return [[SDImageCache1 sharedImageCache] imageFromKey:[url absoluteString]];
}
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed lowPriority:(BOOL)lowPriority
{
    ...
    [[SDImageCache1 sharedImageCache] queryDiskCacheForKey:[url absoluteString] delegate:self userInfo:info];
}
...
- (void)cancelForDelegate:(id<SDWebImageManagerDelegate>)delegate
{
    ...
    {
        SDWebImageDownloader1 *downloader = [downloaders objectAtIndex:idx] ;
...
- (void)imageCache:(SDImageCache1 *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    SDWebImageDownloader1 *downloader = [downloaderForURL objectForKey:url];

    if (!downloader)
    {
        downloader = [SDWebImageDownloader1 downloaderWithURL:url delegate:self userInfo:nil lowPriority:lowPriority];
...
- (void)imageDownloader:(SDWebImageDownloader1 *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        SDWebImageDownloader1 *aDownloader = [downloaders objectAtIndex:uidx];
    ...
    if (image)
    {
        ...
        [[SDImageCache1 sharedImageCache] storeImage:image
