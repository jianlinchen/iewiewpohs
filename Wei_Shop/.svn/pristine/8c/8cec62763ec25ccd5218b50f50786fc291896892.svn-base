...
//    [self changeTheDateString:dict[@"strTime"]];
...
{
    NSString *subString = [Str substringWithRange:NSMakeRange(0, 15)];
...
- (void)minuteOffSetStart:(NSString *)start end:(NSString *)end
{
    ...
    NSString *subStart = [start substringWithRange:NSMakeRange(0, 15)];
    NSDate *startDate = [NSDate dateFromString:subStart withFormat:@"yyyy-MM-dd HH:mm:ss"];
    
    NSString *subEnd = [end substringWithRange:NSMakeRange(0, 15)];
