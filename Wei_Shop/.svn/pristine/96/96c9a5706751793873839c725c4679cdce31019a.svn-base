...
//        [self addSubview:_activityView];
        ...
        imageView.backgroundColor = kPRBGColor;
        imageArr = [[NSMutableArray alloc]initWithObjects:@"1_head",@"2_head",@"3_head",@"4_head",@"5_head", nil];
        
        [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(pao) userInfo:nil repeats:YES];
...
- (void)layouts {
    ...
    imageView.frame = CGRectMake(0, y, size.width, kPRLabelHeight*2);
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            if (self.isAtTop) {
                ...
                _stateLabel.text = NSLocalizedString(@"加载更多", @"");
            }
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    ...
    float h=scrollView.contentSize.height - scrollView.bounds.size.height;
        //加载...
//    NSLog(@"-------------------%f++++++++++++++%f",pt.y,h);
...
//    NSLog(@"%f>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",yMargin);
    ...
    } else if ( yMargin>=0/*yMargin>kPROffsetY*/){  //footer totally appeared
...
- (void)flashMessage:(NSString *)msg{
    ...
    [UIView animateWithDuration:.4f animations:^{
        ...
        rect.origin.y -= 20;
        [UIView animateWithDuration:.2f delay:1.0f options:UIViewAnimationOptionCurveLinear animations:^{
            _msgLabel.frame = rect;
        } completion:^(BOOL finished){
            [_msgLabel removeFromSuperview];
            _msgLabel = nil;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    // Delete by Suihuanhuan at 2012-10-22
